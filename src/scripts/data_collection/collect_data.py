...
def callback_cam(msg):
    ...
    if utils.IMG_DIR and isMoving:
        try:
            ...
            if utils.N_WAYS == 5:
                label = get_direction_from_yaw_5_way(utils.CURR_YAW,utils.CURR_YAW - utils.PREV_YAW)
            elif utils.N_WAYS == 3:
                label = get_direction_from_yaw(utils.CURR_YAW, utils.CURR_YAW - utils.PREV_YAW)
            utils.LABEL_BUFFER.append([utils.PERSIST_INDEX, utils.CURR_YAW,label])
            utils.POSE_BUFFER.append([utils.PERSIST_INDEX, list(utils.CURR_POSI) + list(utils.CURR_ORI)])

            logger.info("Collecting data (Buffer: %d) Last Direction (%d) ..",len(utils.IMG_BUFFER), label)
...
if __name__=='__main__':
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "", ["experiment-type=", "data-dir=", "n-ways="])
