...
def get_column_plot_with_subset(ax, environment, multi_summary,naive_summary,sub_titles,sub_y_labels,sub_x_labels, title,fontsize,title_fontsize,legend_loc):
    ...
    mult_rects = ax.bar(bar_indices, multi_data, col_width, color='r',label='CerberusCNN')
    naiv_rects = ax.bar(bar_indices + col_width, naive_data, col_width, color='b',label='CNN (Vanilla)')
...
if __name__ == '__main__':
    ...
    ax1_sub_x_labels = ['Acc-NC-H', 'Acc-NC-S',
                                 'Acc-C-H','Acc-C-S']
    ax1_sub_y_labels = 'Test Accuracy (%)'
    fontsize = 22

    main_title = "CerberusCNN vs Vanilla CNN Performance\n(Apartment-2 Test Dataset)"
    mul_rects, nai_rects = get_column_plot_with_subset(axarr[0], environment, multi_dict, naive_dict,
                                                       ax_1sub_titles, ax1_sub_y_labels, ax1_sub_x_labels,
                                                       'Collision and Non-Collision Accuracies', fontsize, fontsize,1)
    axarr[0].plot(np.arange(5),[33.3 for _ in range(5)],c='gray',linestyle='--',linewidth=2)
    ...
    ax2_x_labels = ['Prec-NC-TL', 'Prec-NC-GS', 'Prec-NC-TR']
    ax2_y_labels = ''
    get_column_plot_with_subset(axarr[1],environment,multi_dict,naive_dict,ax2_sub_titles,ax2_y_labels,ax2_x_labels,'Precision for Non-Collision Data',fontsize,fontsize,9)

    ax3_sub_titles = ['Precision-C-L', 'Precision-C-S', 'Precision-C-R']
    ax3_x_labels = ['Prec-C-TL', 'Prec-C-GS', 'Prec-C-TR']
    ax3_y_labels = ''
    get_column_plot_with_subset(axarr[2], environment, multi_dict, naive_dict, ax3_sub_titles, ax3_y_labels,
                                ax3_x_labels, 'Precision for Collision Data', fontsize, fontsize,9)
