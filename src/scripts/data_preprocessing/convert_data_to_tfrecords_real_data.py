...
def save_training_data(test_range):
        ...
        try:
            with open(data_folder + os.sep + drive_angle_filename) as f:
                ...
                for line in f:
                    ...
                    if int(txt_tokens[0]) not in list(range(*test_range)):
...
def save_testing_data(test_range):
    ...
    for is_bump_data, data_folder in zip(is_bump_list, data_folders_list):
        ...
        try:
            with open(data_folder + os.sep + drive_angle_filename) as f:
                ...
                for line in f:
                    txt_tokens = line.split(':')
                    if int(txt_tokens[0]) in list(range(*test_range)):
